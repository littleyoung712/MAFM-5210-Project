...
class Kline(object):  # 返回
    PATH = 'sqlite:///' + r"../../data/Backtest/ashare_price.db"  # path of db storage
    Data = None
    ...
    def load_data(self, codes: Optional[List[str]] = None, dates: Optional[List[str]] = None) -> pd.DataFrame:
        ...
        data.columns = ['st_code', 'trade_date', 'close_adj', 'trade_state','open', 'close', 'avg_price','high','low']
        return data
    ...
    def get_data(self, indicator: str) -> Optional[pd.DataFrame]:
        try:
            ...
            print("Indicator must be one of 'close_adj','trade_state','open','close','avg_price','high','low'")
